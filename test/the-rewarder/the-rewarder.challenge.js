...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    ...
    const TOKENS_FLASH_LOAN = ethers.utils.parseEther("10000"); // 10k  tokens
    const AttackerDeployed = await ethers.getContractFactory(
      "AttackRewarder",
      attacker
    );
    const attackContract = await AttackerDeployed.deploy(
      this.flashLoanPool.address,
    ...
    await attackContract.start(TOKENS_IN_LENDER_POOL);
  });
